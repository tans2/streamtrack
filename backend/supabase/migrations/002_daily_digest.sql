...
DROP POLICY IF EXISTS "Service role full access to pending_notification_events" ON pending_notification_events;
...
DROP POLICY IF EXISTS "Service role full access to digest_log" ON digest_log;
CREATE POLICY "Service role full access to digest_log"
  ON digest_log FOR ALL TO service_role
  USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Users can view own digests" ON digest_log;
